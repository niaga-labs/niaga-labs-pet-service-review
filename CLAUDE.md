...
Needs the dev-infra stack: Postgres database `kilat_review`, Kafka on `localhost:9092` -> `cd ~/Documents/dev-infra; ./dev.ps1 up kilat`.
...
No test files in any package. `go test ./...` has nothing to run - worth fixing, given the invariants this schema enforces.

## Where things are

- `cmd/server` - `cmd/migrate` - `internal/domain/review` holds the invariants (rating 1-5, known reviewee type, no self-review) - `internal/repository`

## Worth knowing

- Until KPD-58 this repo had no migrations/ directory at all and could not boot outside development. The SQL CHECK constraints mirror NewReview deliberately - keep them in step.
